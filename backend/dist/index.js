...
const toSlug = (value) => value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
const resolveConfigPath = (inputPath) => path_1.default.isAbsolute(inputPath) ? inputPath : path_1.default.resolve(process.cwd(), inputPath);
const loadServersConfig = async () => {
    const resolvedPath = resolveConfigPath(configPath);
    const raw = await promises_1.default.readFile(resolvedPath, "utf-8");
...
const getServerById = (id) => servers.find((server) => server.id === id);
const normalizeRelativePath = (inputPath) => inputPath.replace(/\\/g, "/").replace(/^\/+/, "");
const resolveServerPath = (server, relPath) => {
    const root = path_1.default.resolve(server.path);
    const safeRel = normalizeRelativePath(relPath);
    const full = path_1.default.resolve(root, safeRel);
    const relative = path_1.default.relative(root, full);
    if (relative.startsWith("..") || path_1.default.isAbsolute(relative)) {
        throw new Error("Invalid path");
    }
    return { root, full, rel: safeRel };
};
const toPosixPath = (base, name) => {
    if (!base) {
        return name;
    }
...
const normalizeLinuxPath = (value) => value.replace(/\\/g, "/");
const findProcessByPath = async (serverPath) => {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            try {
                const cmdlineRaw = await promises_1.default.readFile(cmdlinePath, "utf-8");
...
const readProcTimes = async (pid) => {
    const statRaw = await promises_1.default.readFile(`/proc/${pid}/stat`, "utf-8");
...
const readTotalCpuTime = async () => {
    const statRaw = await promises_1.default.readFile("/proc/stat", "utf-8");
...
const readProcessMemoryMB = async (pid) => {
    const statusRaw = await promises_1.default.readFile(`/proc/${pid}/status`, "utf-8");
...
const getProcessStats = async (pid) => {
    try {
        ...
        if (previous) {
            ...
            if (totalDelta > 0 && procDelta >= 0) {
                cpuPercent = Math.min(100, Math.round((procDelta / totalDelta) * 100 * os_1.default.cpus().length));
            }
        }
        return {
            pid,
...
const updateLiveStatus = async (server, state) => {
    ...
    let rcon = null;
    try {
        rcon = await rcon_client_1.Rcon.connect({
            host,
            port,
...
app.get("/api/servers/:id/files", async (req, res) => {
    ...
    try {
        const stat = await promises_1.default.stat(resolved.full);
        if (!stat.isDirectory()) {
            return res.status(400).json({ error: "Not a directory." });
        }
        const entries = await promises_1.default.readdir(resolved.full, { withFileTypes: true });
        const payload = await Promise.all(entries.map(async (entry) => {
            const entryPath = toPosixPath(resolved.rel, entry.name);
            const fullEntryPath = path_1.default.resolve(resolved.full, entry.name);
            if (entry.isFile()) {
                const fileStat = await promises_1.default.stat(fullEntryPath);
                return {
                    name: entry.name,
                    path: entryPath,
                    type: "file",
...
app.get("/api/servers/:id/files/content", async (req, res) => {
    ...
    try {
        ...
        const content = await promises_1.default.readFile(resolved.full, "utf-8");
...
app.post("/api/servers/:id/files/content", async (req, res) => {
    ...
    try {
        const dirPath = path_1.default.dirname(resolved.full);
        await promises_1.default.mkdir(dirPath, { recursive: true });
        await promises_1.default.writeFile(resolved.full, content, "utf-8");
        return res.json({ ok: true });
    }
    catch {
...
app.post("/api/servers/:id/files/dir", async (req, res) => {
    if (!serversLoaded) {
        return res.status(500).json({ error: "No servers configured." });
    }
    const server = getServerById(getRouteParam(req, "id"));
    ...
    try {
        await promises_1.default.mkdir(resolved.full, { recursive: true });
        return res.json({ ok: true });
    }
    catch {
...
app.post("/api/servers/:id/files/rename", async (req, res) => {
    ...
    try {
        await promises_1.default.rename(fromResolved.full, toResolved.full);
        return res.json({ ok: true });
    }
    catch {
...
app.delete("/api/servers/:id/files", async (req, res) => {
    ...
    try {
        await promises_1.default.rm(resolved.full, { recursive: true, force: true });
        return res.json({ ok: true });
    }
    catch {
...
app.post("/api/servers/:id/files/upload", async (req, res) => {
    ...
    try {
        ...
        await promises_1.default.writeFile(resolved.full, buffer);
        return res.json({ ok: true });
    }
    catch {
...
app.get("/api/servers/:id/files/download", async (req, res) => {
    ...
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="${path_1.default.basename(resolved.full)}"`);
        const buffer = await promises_1.default.readFile(resolved.full);
